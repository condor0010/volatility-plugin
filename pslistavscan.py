...
class PsListAvScan(interfaces.plugins.PluginInterface, timeliner.TimeLinerInterface):
    ...
    def process_avscan(
        ...
        try:
            ...
            for offset, data in dos_header.reconstruct():
                file_bytes += data
            ret = cd.instream(BytesIO(file_bytes))["stream"]
            scan_report = ret[0]
            scan_desc = ret[1]
            if (scan_desc == None):
                scan_desc = "None"
    ...
    def _generator(self, cd):
        ...
        for proc in self.list_processes(
            ...
            avscan_report = "Disabled"
            avscan_desc = "Disabled"
            try:
                if self.config["dump"]:
                ...
                if self.config["avscan"]:
                    avscan_data = self.process_avscan(
                        self.context,
                        kernel.symbol_table_name,
                        pe_table_name,
                        proc,
                    ...
                    #print(f'{file_handle[0]}  {file_handle[1]}')
                    if avscan_data:
                        avscan_report = avscan_data[0]
                        avscan_desc = avscan_data[1]
