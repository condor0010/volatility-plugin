...
def check_ip_malicious(ip_address):
    ...
    data = response.json()
    detected_services = data.get('data', {}).get('attributes', {}).get('last_analysis_results', {})

    for service, info in detected_services.items():
        if info.get('result') == 'malicious':
    ...
    def _generator(self, show_corrupt_results: Optional[bool] = None):
        ...
        for netw_obj in self.scan(
            ...
            if isinstance(netw_obj, network._UDP_ENDPOINT):
                ...
                try:
                    cip = check_ip_malicious(netw_obj.get_remote_address()) or renderers.UnreadableValue()
