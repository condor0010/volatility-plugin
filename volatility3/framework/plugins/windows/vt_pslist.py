...
API_KEY = open('/opt/virus_total_key.txt').readline().strip('\n')

class VTScan(interfaces.plugins.PluginInterface, timeliner.TimeLinerInterface):
    ...
    def get_requirements(cls):
        return [
            ...
            requirements.BooleanRequirement(
                name="vtscan",
                description="Anti-virus scan listed processes",
                default=False,
                optional=True,
            ),
        ]
    ...
    def process_vtscan(
        ...
        try:
            ...
            array_of_bytes = bytearray(b'')
            byte_io = io.BytesIO(array_of_bytes)
            ...
            for offset, data in dos_header.reconstruct():
                byte_io.seek(offset)
                byte_io.write(data)
            filehash.update(byte_io.getvalue())

            url = f'https://www.virustotal.com/api/v3/files/{filehash.hexdigest()}'
            headers = {'x-apikey': API_KEY}
            response = requests.get(url, headers=headers)
            data = json.loads(response.text)
            try:
                if data['error']['message'] == 'Quota exceeded':
                    ret_status = 'Quota Exceeded'
            except:
                pass
            if response.status_code == 200:
                ret_status = data['data']['attributes']['popular_threat_classification']['suggested_threat_label']#['data']['attributes']['type_description']['suggested_threat_label']
        except Exception as excp:
            vollog.debug(f"Unable to dump PE with pid {proc_id}: {excp}")
    ...
    def _generator(self):
        ...
        for proc in self.list_processes(
            ...
            try:
                ...
                if self.config["vtscan"]:
                    vtscan_data = self.process_vtscan(
                        self.context,
                        kernel.symbol_table_name,
                        pe_table_name,
                        proc,
